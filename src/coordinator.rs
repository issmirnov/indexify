...
impl Coordinator {
    ...
    pub async fn list_content(
        ...
        let content = self.shared_state.list_content(namespace)?;
...
mod tests {
    ...
    async fn test_run_content_through_multiple_extraction_graphs() -> Result<(), anyhow::Error> {
        ...
        let extractor = mock_extractor();
        coordinator
            .register_executor("localhost:8950", executor_id_1, extractor)
            .await?;
        coordinator.run_scheduler().await?;
        ...
        coordinator
            .register_executor("localhost:8951", executor_id_2, extractor)
            .await?;
        coordinator.run_scheduler().await?;
    ...
    async fn test_create_and_read_content_tree() -> Result<(), anyhow::Error> {
        ...
        coordinator
            .register_executor("localhost:8956", executor_id_1, extractor_1.clone())
            .await?;
        ...
        coordinator
            .register_executor("localhost:8957", executor_id_2, extractor_2.clone())
            .await?;
    ...
    async fn test_match_tombstoned_content() -> Result<(), anyhow::Error> {
        ...
        let parent_content_internal =
            coordinator.external_content_metadata_to_internal(vec![parent_content.clone()])?;
        let parent_content_internal = parent_content_internal.first().unwrap();
